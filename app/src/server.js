...
const api_key_secret = process.env.API_KEY_SECRET || "गुरुkool_default_secret";
...
app.get("/join/", (req, res) => {
  if (Object.keys(req.query).length > 0) {
    ...
    /* 
            http://localhost:3000/join?room=test&name=गुरुkool&audio=1&video=1&notify=1
            https://गुरुkool.up.railway.app/join?room=test&name=गुरुkool&audio=1&video=1&notify=1
            https://गुरुkool.herokuapp.com/join?room=test&name=गुरुkool&audio=1&video=1&notify=1
        */
...
app.post([apiBasePath + "/meeting"], (req, res) => {
  ...
  if (authorization != api_key_secret) {
    log.debug("गुरुkool get meeting - Unauthorized", {
      header: req.headers,
      body: req.body,
    });
  ...
  log.debug("गुरुkool get meeting - Authorized", {
    header: req.headers,
    body: req.body,
    meeting: meetingURL,
  });
